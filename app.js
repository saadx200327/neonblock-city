(() => {
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio || 1, isCoarse ? 1.25 : 1.6));
  ...
  const state = {
    cash: 100, xp: 0, level: 1, wanted: 0, slot: 'slot1', paused: false, dirty: false,
    player: { pos: new THREE.Vector3(0, 1.1, 0), velY: 0, yaw: 0, onGround: true },
    keys: new Set(), joystick: { x: 0, y: 0, active: false }, sprint: false, pointerLook: { active: false, id: null, x: 0 },
    chunkSize: 72, chunkRadius: isCoarse ? 1 : 2, chunks: new Map(), npcs: [], pickups: [], vehicles: [], lots: new Map(), ownedLots: new Set(), takenPickups: new Set(), activeVehicle: null,
    missions: [
      ...
      { id: 'owner', name: 'First Deed', reward: 220, xp: 95, text: 'Buy any city lot.', need: 1 }
    ],
  ...
  const pickupId = (x, z) => `${Math.round(x * 10)}:${Math.round(z * 10)}`;
  const markDirty = () => { state.dirty = true; };

  function spawnPickup(x, z, group, chunk) {
    const id = pickupId(x, z);
    if (state.takenPickups.has(id)) return;
    const mesh = new THREE.Mesh(new THREE.BoxGeometry(1.1, 1.1, 1.1), mats.cash);
    mesh.position.set(x, 0.8, z); mesh.castShadow = !isCoarse; group.add(mesh);
    state.pickups.push({ id, mesh, chunk, value: 20 + Math.floor(rand(x, z, 1) * 45), taken: false });
  }
  function spawnVehicle(x, z, group, chunk) {
    const car = new THREE.Group();
    const base = new THREE.Mesh(new THREE.BoxGeometry(3.7, 0.85, 5.2), mats.car); base.position.y = 0.55; base.castShadow = !isCoarse; car.add(base);
    const cabin = new THREE.Mesh(new THREE.BoxGeometry(2.3, 0.9, 2.4), mats.car); cabin.position.y = 1.25; cabin.castShadow = !isCoarse; car.add(cabin);
  ...
  function spawnNpc(x, z, group, chunk) {
    ...
    mesh.position.set(x, 0.85, z); mesh.castShadow = !isCoarse; group.add(mesh);
  ...
  function spawnLot(x, z, group) {
    ...
    const owned = state.ownedLots.has(id);
    if (owned) mesh.material = mats.owned;
    state.lots.set(id, { id, mesh, price, owned });
  }
  ...
  function addChunk(cx, cz) {
    ...
    const buildingCount = isCoarse ? 6 : 9;
    for (let i = 0; i < buildingCount; i++) {
      ...
      b.position.set(bx, h / 2, bz); b.castShadow = !isCoarse; b.receiveShadow = true; group.add(b);
    }
    ...
    if (!isCoarse && rand(cx, cz, 66) > 0.58) spawnNpc(x0 + rand(cx, cz, 67) * 48 - 24, z0 + rand(cx, cz, 68) * 48 - 24, group, key);
  ...
  function removeChunk(key) {
    ...
    for (const [id, lot] of state.lots) if (lot.mesh.parent === group) state.lots.delete(id);
  }
  ...
  function updatePlayer(dt) {
    ...
    if (state.activeVehicle) {
      ...
      if (v.gas > 0) {
        ...
        if (Math.abs(v.speed) > 4) markDirty();
      } else v.speed *= 0.96;
      ...
      if (move.x || move.z) { state.player.yaw = Math.atan2(move.x, move.z); state.player.pos.x += move.x * speed * dt; state.player.pos.z += move.z * speed * dt; markDirty(); }
  ...
  function jump() { if (!state.activeVehicle && state.player.onGround) { state.player.velY = 13; state.player.onGround = false; markDirty(); } }
  function interact() {
    const p = state.player.pos;
    if (state.activeVehicle) { state.activeVehicle.mesh.position.copy(p).add(new THREE.Vector3(Math.sin(state.activeVehicle.yaw) * 4, 0, Math.cos(state.activeVehicle.yaw) * 4)); state.activeVehicle = null; popup('Exited vehicle'); markDirty(); return; }
    ...
    for (const lot of state.lots.values()) {
      ...
      if (d < 7) {
        if (!lot.owned && state.cash >= lot.price) {
          state.cash -= lot.price; lot.owned = true; state.ownedLots.add(lot.id); lot.mesh.material = mats.owned; popup(`Lot owned -$${lot.price}`); markDirty();
          if (state.mission?.id === 'owner') completeMission(); else saveGame({ silent: true });
  ...
  function updateWorld(dt) {
    ...
    for (const p of state.pickups) if (!p.taken) {
      ...
      if (p.mesh.position.distanceTo(state.player.pos) < 2.4) {
        p.taken = true; p.mesh.visible = false; state.cash += p.value; state.xp += 8; state.takenPickups.add(p.id); markDirty();
        if (state.mission?.id === 'collector') state.missionProgress++;
        popup(`+$${p.value}`);
        if (state.mission?.id === 'collector' && state.missionProgress >= state.mission.need) completeMission();
      }
    }
  ...
  function setMission(m) { state.mission = { ...m }; state.missionProgress = 0; if (m.id === 'driver' && state.activeVehicle) state.activeVehicle.driven = 0; popup(`Mission started: ${m.name}`); markDirty(); closeAllMenus(); }
  function completeMission() { const m = state.mission; if (!m) return; state.cash += m.reward; state.xp += m.xp; popup(`Mission complete: +$${m.reward} +${m.xp}XP`); state.mission = null; state.missionProgress = 0; markDirty(); saveGame({ silent: true }); }
  ...
  function drawMinimap() {
    ...
    for (const lot of state.lots.values()) if (lot.owned) { const x = 80 + (lot.mesh.position.x - px) * 0.35, y = 80 + (lot.mesh.position.z - pz) * 0.35; if (x > 0 && x < 160 && y > 0 && y < 160) { ctx.fillStyle = '#5ef38c'; ctx.fillRect(x - 2, y - 2, 4, 4); } }
  ...
  function savePayload() { return { version: 3, cash: state.cash, xp: state.xp, level: state.level, wanted: state.wanted, player: { x: state.player.pos.x, y: state.player.pos.y, z: state.player.pos.z, yaw: state.player.yaw }, lots: [...state.ownedLots], takenPickups: [...state.takenPickups].slice(-200), mission: state.mission, missionProgress: state.missionProgress, savedAt: new Date().toISOString() }; }
  async function saveGame(opts = {}) { const slot = typeof opts === 'string' ? opts : (opts.slot || state.slot); const silent = typeof opts === 'object' && opts.silent; try { const payload = savePayload(); localStorage.setItem(`neonblock:${slot}`, JSON.stringify(payload)); if (state.cloud?.save) await state.cloud.save(slot, payload); state.lastSave = performance.now(); state.dirty = false; if (!silent) popup('Game saved'); } catch (e) { hud.error.textContent = e.message || String(e); } }
  async function loadGame(slot = state.slot) { try { let raw = localStorage.getItem(`neonblock:${slot}`); if (!raw && state.cloud?.load) { const cloud = await state.cloud.load(slot); if (cloud) raw = JSON.stringify(cloud); } if (!raw) return; applySave(JSON.parse(raw)); popup('Game loaded'); } catch (e) { hud.error.textContent = e.message || String(e); } }
  function applySave(data) {
    state.cash = Number(data.cash || 100); state.xp = Number(data.xp || 0); state.level = Number(data.level || 1); state.wanted = Number(data.wanted || 0);
    state.ownedLots = new Set(data.lots || []); state.takenPickups = new Set(data.takenPickups || []);
    if (data.player) { state.player.pos.set(data.player.x || 0, Math.max(1.1, data.player.y || 1.1), data.player.z || 0); state.player.yaw = data.player.yaw || 0; }
    if (data.mission?.id) { const m = state.missions.find(x => x.id === data.mission.id); state.mission = m ? { ...m } : null; state.missionProgress = Number(data.missionProgress || 0); }
    for (const lot of state.lots.values()) { lot.owned = state.ownedLots.has(lot.id); lot.mesh.material = lot.owned ? mats.owned : mats.lot; }
    for (const p of state.pickups) if (state.takenPickups.has(p.id)) { p.taken = true; p.mesh.visible = false; }
    state.dirty = false;
  }
  function exportSave() { $('export-json').value = JSON.stringify(savePayload(), null, 2); }
  function importSave() { try { const txt = $('export-json').value.trim(); if (!txt) return popup('Paste save JSON first'); applySave(JSON.parse(txt)); saveGame({ silent: true }); popup('Imported save'); } catch (e) { hud.error.textContent = e.message || String(e); popup('Invalid JSON'); } }
  ...
  function unstuck() { state.activeVehicle = null; state.player.pos.y = 1.1; state.player.velY = 0; markDirty(); popup('Unstuck'); }
  function bindControls() {
    addEventListener('keydown', e => { if (['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code)) e.preventDefault(); state.keys.add(e.code); if (e.code === 'Space') jump(); if (e.code === 'KeyE') interact(); if (e.code === 'KeyM') openMissionBoard(); if (e.code === 'Escape') togglePause(); if (e.code === 'KeyR') unstuck(); if (e.code === 'F3') document.body.classList.toggle('debug'); });
    addEventListener('keyup', e => state.keys.delete(e.code));
    const down = (id, fn) => { const el = $(id); if (!el) return; el.addEventListener('pointerdown', e => { e.preventDefault(); fn(true); }); el.addEventListener('pointerup', e => { e.preventDefault(); fn(false); }); el.addEventListener('pointercancel', e => { e.preventDefault(); fn(false); }); el.addEventListener('lostpointercapture', () => fn(false)); };
    ...
    const resetJoy = () => { state.joystick = { x: 0, y: 0, active: false }; if (stick) stick.style.transform = 'translate(0,0)'; };
    function handleJoy(e) { if (!state.joystick.active || !joy || !stick) return; const r = joy.getBoundingClientRect(), dx = e.clientX - (r.left + r.width / 2), dy = e.clientY - (r.top + r.height / 2), max = r.width * 0.36, len = Math.min(max, Math.hypot(dx, dy)), a = Math.atan2(dy, dx), sx = Math.cos(a) * len, sy = Math.sin(a) * len; state.joystick.x = sx / max; state.joystick.y = sy / max; stick.style.transform = `translate(${sx}px,${sy}px)`; }
    joy?.addEventListener('pointerdown', e => { e.preventDefault(); joy.setPointerCapture(e.pointerId); state.joystick.active = true; handleJoy(e); }); joy?.addEventListener('pointermove', handleJoy); joy?.addEventListener('pointerup', resetJoy); joy?.addEventListener('pointercancel', resetJoy); joy?.addEventListener('lostpointercapture', resetJoy);
    canvas.addEventListener('pointerdown', e => { if (e.target !== canvas || state.paused || e.pointerType === 'mouse') return; state.pointerLook = { active: true, id: e.pointerId, x: e.clientX }; canvas.setPointerCapture(e.pointerId); });
    canvas.addEventListener('pointermove', e => { if (!state.pointerLook.active || e.pointerId !== state.pointerLook.id) return; const dx = e.clientX - state.pointerLook.x; state.pointerLook.x = e.clientX; state.player.yaw -= dx * 0.006; if (state.activeVehicle) state.activeVehicle.yaw = state.player.yaw; });
    ['pointerup','pointercancel','lostpointercapture'].forEach(type => canvas.addEventListener(type, () => { state.pointerLook.active = false; }));
    ...
    document.querySelectorAll('.btn-save-slot').forEach(b => b.onclick = () => { state.slot = b.dataset.slot; saveGame({ slot: state.slot }); }); document.querySelectorAll('.btn-load-slot').forEach(b => b.onclick = () => { state.slot = b.dataset.slot; loadGame(state.slot); });
    $('graphics-quality').onchange = e => { const q = e.target.value, ratio = q === 'low' ? 1 : q === 'high' ? Math.min(devicePixelRatio || 1, 2) : Math.min(devicePixelRatio || 1, isCoarse ? 1.25 : 1.6); state.chunkRadius = q === 'low' ? 1 : (isCoarse ? 1 : 2); renderer.shadowMap.enabled = q !== 'low' && !isCoarse; renderer.setPixelRatio(ratio); popup(`Graphics: ${q}`); };
    document.addEventListener('visibilitychange', () => { if (document.hidden && state.dirty) saveGame({ silent: true }); });
  }
  ...
  function loop() {
    ...
    updatePlayer(dt); updateWorld(dt); updateCamera(dt); updateHud(dt);
    if (state.dirty && performance.now() - state.lastSave > 30000) saveGame({ silent: true });
    renderer.render(scene, camera);
  }
  async function start() { bindControls(); addEventListener('resize', resize); streamWorld(); await initCloud(); await loadGame(state.slot); loading?.classList.add('hidden'); popup('WASD/joystick to move, E to interact, M missions, F3 debug'); loop(); }
  start();
})();