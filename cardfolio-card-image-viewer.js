...
(() => {
  ...
  if (window.cardfolioCardImageViewerVersion) return;
  window.cardfolioCardImageViewerVersion = '20260910-1';

  const STYLE_ID = 'cardfolio-card-image-viewer-css';
  const DIALOG_ID = 'cardfolioCardImageViewer';
  const IMAGE_SELECTOR = '#assetDetailContent .asset-image-wrap img';
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      @media (max-width:720px) {
        #assetDetailContent .asset-image-wrap.cardfolio-image-zoomable { border-radius: 16px; }
  ...
  function prepareZoomTargets(root = document) {
    root.querySelectorAll?.(IMAGE_SELECTOR).forEach((image) => {
      const wrap = image.closest('.asset-image-wrap');
      if (!wrap || wrap.dataset.cardfolioImageViewerReady === '1') return;
      wrap.dataset.cardfolioImageViewerReady = '1';
  ...
  document.addEventListener('keydown', (event) => {
    ...
    const wrap = event.target?.closest?.('#assetDetailContent .asset-image-wrap.cardfolio-image-zoomable');
    if (!wrap) return;
