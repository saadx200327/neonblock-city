(() => {
  ...
  function countOwnedLots(p = player()) {
  ...
  function makePanel() {
    ...
    panel.innerHTML = `
      <h3>Backpack</h3>
      <p id="inv-summary">Loading inventory...</p>
      <div class="inv-grid">
        <div class="inv-card"><b>Medkits</b><span id="inv-medkit">0</span></div>
        <div class="inv-card"><b>Fuel Cells</b><span id="inv-fuel">0</span></div>
        <div class="inv-card"><b>City Passes</b><span id="inv-pass">0</span></div>
      </div>
      <p id="inv-hint"></p>
      <div class="inv-row">
        <button type="button" id="inv-use-medkit">Repair</button>
        <button type="button" id="inv-use-fuel">Fuel</button>
        <button type="button" id="inv-use-pass">Cash Pass</button>
        <button type="button" id="inv-save">Save</button>
        <button type="button" id="inv-copy">Copy QA</button>
      </div>`;
  ...
  function label(id) {
    return id === 'medkit' ? 'medkit' : id === 'fuel' ? 'fuel cell' : 'city pass';
  }
  ...
  function trackRewards() {
    ...
    if (collectedEstimate > 0) addItem('fuel', Math.min(2, collectedEstimate), 'Crate route bonus');
    if (ownedLots > Number(state.lastOwnedLots || 0)) addItem('cityPass', ownedLots - Number(state.lastOwnedLots || 0), 'Property owner bonus');
    if (level > Number(state.lastLevel || 1)) addItem('medkit', level - Number(state.lastLevel || 1), 'Level-up safety bonus');
    state.lastCrates = crates;
    state.lastOwnedLots = ownedLots;
  ...
  function useMedkit() {
    ...
    car.userData.hp = Math.max(Number(car.userData.hp || 0), 85);
    state.items.medkit -= 1;
    state.totalUsed += 1;
    message('Vehicle repaired to safe HP.');
    quickSave(false);
  }
  ...
  function useFuel() {
    ...
    car.userData.gas = Math.max(Number(car.userData.gas || 0), 80);
    state.items.fuel -= 1;
    state.totalUsed += 1;
    message('Vehicle refueled from backpack.');
    quickSave(false);
  }
  ...
  function usePass() {
    ...
    p.cash = Number(p.cash || 0) + 75;
    p.xp = Number(p.xp || 0) + 15;
    state.items.cityPass -= 1;
    state.totalUsed += 1;
    message('City pass redeemed for $75 and 15 XP.');
    quickSave(false);
  }
  ...
  function quickSave(show = true) {
    try {
      ...
      if (show) state.lastMessage = 'Inventory and game state saved.';
    } catch (error) {
  ...
  function report() {
    const snap = snapshot();
    const p = snap?.player;
    const car = activeVehicle(p);
    return {
      feature: 'Backpack inventory polish',
      items: state.items,
      totalUsed: state.totalUsed,
      cash: Math.floor(p?.cash || 0),
      xp: Math.floor(p?.xp || 0),
      level: Math.floor(p?.level || 1),
      ownedLots: countOwnedLots(p),
      vehicle: car?.userData?.name || 'On foot',
      vehicleHp: car ? Math.floor(car.userData.hp || 0) : null,
      vehicleGas: car ? Math.floor(car.userData.gas || 0) : null,
      chunks: snap?.chunks ?? 0,
      cratesVisible: snap?.crates ?? 0,
  ...
  async function copyReport() {
    ...
    catch { state.lastMessage = 'Backpack QA report saved locally.'; }
    saveState();
    render();
  }
  ...
  function boot() {
    ...
    document.addEventListener('keydown', (event) => {
      if (event.code === 'Digit1' && !event.repeat && !/INPUT|TEXTAREA|SELECT/.test(event.target?.tagName || '')) togglePanel();
    });
